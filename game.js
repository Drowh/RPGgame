...
// Функция для создания копии врага из шаблона
const createEnemyFromTemplate = (template) => {
  const enemy = {
    name: template.name,
    hp: template.hp,
  ...
  // Копируем способности, если они есть
  if (template.abilities) {
    enemy.abilities = template.abilities.map((ability) => ({
      name: ability.name,
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  activeQuestsDiv = document.getElementById("active-quests");
  ...
  const calculateDamage = (attacker, defender) => {
    ...
    // Гарантируем минимальный урон для врагов
    if (damage <= 0 && attacker !== player) {
      damage = Math.floor(Math.random() * 3) + 1; // Минимальный урон 1-3 для врагов
    } else {
  ...
  const updateUI = () => {
    ...
    if (player) {
      ...
      const statsHp = document.querySelector("#stats-hp");
      const statsMaxHp = document.querySelector("#stats-max-hp");
      const statsMp = document.querySelector("#stats-mp");
      const statsMaxMp = document.querySelector("#stats-max-mp");
      const statsStr = document.querySelector("#stats-str");
      const statsDef = document.querySelector("#stats-def");
      const statsAgi = document.querySelector("#stats-agi");

      if (statsHp) statsHp.textContent = Math.floor(player.hp);
      if (statsMaxHp) statsMaxHp.textContent = player.maxHp;
      if (statsMp) statsMp.textContent = Math.floor(player.mp);
      if (statsMaxMp) statsMaxMp.textContent = player.maxMp;
      if (statsStr) statsStr.textContent = player.str;
      if (statsDef) statsDef.textContent = player.def;
      if (statsAgi) statsAgi.textContent = player.agi;
    }

    // Обновление здоровья врага
    if (currentEnemy && enemyHpText && enemyMaxHpText && enemyHpBar) {
        ...
        currentEnemy.abilities.forEach((ability) => {
          if (Math.random() < ability.chance) {
            if (typeof ability.effect === "function") {
              ...
              console.error("Ошибка в способности врага:", ability.name);
            }
          }
        });
      }
  ...
  const claimQuestReward = (questId) => {
    ...
    if (quest.reward.inventory) {
      Object.keys(quest.reward.inventory).forEach((item) => {
        ...
        addLog(
          `Вы получили ${quest.reward.inventory[item]} ${
            item === "health"
              ? "Зелье здоровья"
              : item === "strength"
              ? "Зелье силы"
    ...
    if (quest.reward.stat) {
      Object.keys(quest.reward.stat).forEach((stat) => {
        ...
        addLog(
          `Ваша характеристика ${stat.toUpperCase()} увеличена на ${
            quest.reward.stat[stat]
          }!`
        );
      });
    }
  ...
  const usePotion = (type) => {
    if (inventory[type] <= 0) {
      addLog(`У вас нет ${getPotionName(type)}!`);
      return false;
    }
    inventory[type]--;
  ...
  // Вспомогательная функция для получения русского названия зелья
  const getPotionName = (type) => {
    const potionNames = {
      health: "Зелья здоровья",
      strength: "Зелья силы",
      defense: "Зелья защиты",
      agility: "Зелья ловкости",
      mana: "Зелья маны",
    };
    return potionNames[type] || "неизвестного зелья";
  };
  ...
  document.getElementById("move-forest").addEventListener("click", () => {
    ...
    currentEnemy = createEnemyFromTemplate(goblinTemplate);
    currentNPC = null;
  ...
  document.getElementById("move-village").addEventListener("click", () => {
    ...
    document.getElementById("move-sfcfik-cave").disabled = false;
  ...
  document.getElementById("move-ruins").addEventListener("click", () => {
    ...
    currentEnemy = createEnemyFromTemplate(skeletonTemplate);
    currentNPC = null;
    changeAvatar(currentEnemy.avatar, currentEnemy.name);
    enemyAvatar.style.display = "block";
    enemyHpBar.style.display = "block";
  ...
  document.getElementById("move-sfcfik-cave").addEventListener("click", () => {
    ...
    currentEnemy = createEnemyFromTemplate(sfchikTemplate);
    currentNPC = null;
    changeAvatar(currentEnemy.avatar, currentEnemy.name);
    enemyAvatar.style.display = "block";
    enemyHpBar.style.display = "block";
  ...
  document.getElementById("useItem").addEventListener("click", () => {
    ...
    items.forEach((item) => {
      if (itemClickHandlers.has(item)) {
        item.removeEventListener("click", itemClickHandlers.get(item));
      ...
      const clickHandler = () => {
        ...
        if (usePotion(type)) {
          ...
          items.forEach((i) => {
            ...
            if (itemClickHandlers.has(i)) {
              i.removeEventListener("click", itemClickHandlers.get(i));
              itemClickHandlers.delete(i);
            }
          });
          updateEffects();
          enemyTurn();
